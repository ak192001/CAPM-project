...
try:
    ...
    for stock in stock_list:
        ...
        beta.df = pd.DataFrame(columns= ['Stock', 'Beta Value'])
        beta.df['Stock'] = beta.keys()
        beta.df['Beta Value'] = [str(round(1,2)) for i in beta.values()]

        with col1:
            st.markdown("### Calculate the beta values")
            st.dataframe(beta.df, use_container_width=True)
        ...
        for stock, value in beta.items():
            return_value.append(str(rf+(value*(rm-rf),2)))
    ...
    st.write('Please Select Proper Input')
