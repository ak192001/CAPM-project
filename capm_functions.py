...
#function to normalize the prices based on the initail price

def normalize(df_2):
    df = df_2.copy()
    for i in df.columns[1:]:
        df[i] = df[i]/df[i][0] # this code states that the first price of the stock will be divide to other prices of the stocks
    return df
